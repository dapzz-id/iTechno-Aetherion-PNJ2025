        ...
        <div class="hidden sm:grid grid-cols-3 gap-6 items-end mb-24">
            @foreach ($top3 as $i => $user)
                <div class="flex flex-col items-center">
                    <div class="w-16 h-16 rounded-lg shadow-lg shadow-[#FFE38A]/80">
                    ...
                    <!-- Podium -->
                    <div
                        class="relative mt-4 @if ($i == 1) w-42 @elseif($i == 0) w-36 @else w-32 @endif">
                        ...
                        <div
                            ...
                            @elseif ($i == 0) bottom-9 left-11
                            @else bottom-7 left-10 @endif
                            text-left text-sm space-y-1">
                            <div><i class="fa-solid fa-heart mr-1 text-red-500"></i> {{ $user['likes'] }}</div>
                            <div><i class="fa-regular fa-images mr-1"></i> {{ $user['posts'] }}</div>
                        </div>
                    </div>
                </div>
            @endforeach
                ...
                <tbody>
                    @foreach ($others as $row)
                        <tr class="bg-white rounded-xl shadow-md">
                            ...
                            <td class="py-3 px-4 text-center mr-1">{{ $row['likes'] }}</td>
